...
# old.fit.hdpx <- function(x, y, folds, list.methods, args.glinternet,
#                          parallel = TRUE, B = NULL, K = NULL, strata = NULL){
#   
#   # Preparation
#   if(is.null(folds)) {
...
#   
#   # here, use do.call(cbind, folds.cv) to cbind all matrices from folds.cv
#   if(length(which(apply(do.call(cbind, folds.cv),1,function(x) length(which(x==2)))==0)) > 0){
#     stop("The folds.cv matrix has rows that are not in the test sample.")
#   }
#   
#   # Option if folds is NULL
...
#   yhat.folds <- lapply(fits, function(x) do.call(cbind, x))
#   
#   # Prediction matrix
#   prediction <- folds.cv[, rep(1:ncol(folds.cv), each=length(list.methods))]
#   prediction[prediction == 1] <- NA
#   prediction[prediction == 0] <- NA
#   
#   seq.methods <- rep(1:length(list.methods), length.out=ncol(prediction))  # Check that this works in all combinations of K and B
#   seq.folds <- rep(1:ncol(folds.cv), each=length(list.methods))
#   
#   for (i in seq_len(ncol(prediction))){
#     prediction[which(prediction[, i]==2), i] <- yhat.folds[[seq.folds[i]]][,seq.methods[i]]
#   }  
#   
#   ret <- list(yhat = prediction,
#               folds = folds,
#               y = y)
#   
#   call <- list("methods" = list.methods)
...
# }
# 
# old.predict.hdpx <- function(object, 
#                              method = c("mean", "lowest.rmse", "superlearner"),
#                              prediction.matrix = NULL, y = NULL, B = NULL, K = NULL,
#                              methods = NULL, folds.m = NULL){
#   
...
#   # Compute RMSE for each folds
#   rmse <- function(yhat, y){sqrt(mean((yhat-y)^2, na.rm = T)) }
#   rmse.list <- apply(prediction.matrix, 2, rmse, y = y) # CHECK
#   
#   # Variant 1: take the row mean
#   if(method == "mean"){
#     yhat <- apply(prediction.matrix, 1, mean, na.rm = T) %>% 
#       as.matrix()
#   }
#   
#   # Variant 2: take the observation from the fold and method with the lowest RMSE
...
#   if(method == "superlearner"){
#     
#     which(duplicated(cbind(folds.m, folds.m, folds.m), MARGIN = 2))
#     anyDuplicated(cbind(folds.m, folds.m), MARGIN = 2)
#     
#     yhat <- matrix(nrow = nrow(prediction.matrix), ncol = 1)
#     
#     for (jj in 1:ncol(folds.m)){
#       jjj <- which(folds.m[, jj] == 2)
#       predictors <- prediction.matrix[jjj, , drop = F]
#       predictors <- predictors[, apply(predictors, 2, function(x) !any(is.na(x))), drop = F]
#       
#       y.fold <- y[jjj, , drop = F]
#       
#       dim(predictors); dim(y.fold)
#       
#       colnames(predictors) <- paste0("method", 1:ncol(predictors))
#       w.yhat <- ensemble(predictors = predictors, y = y.fold, k = n.k)
#       
#       yhat[jjj,] <- w.yhat$weighted.yhat
#       
#     }
#     # for(jj in split(1:(K*n.k*B), rep(1:(K*n.k*B), each = n.k))){
#     # for(jj in split(1:(K*n.k), rep(1:K, each = n.k))){
#     #   predictors <- prediction.matrix[, jj] #select columns
#     #   predictors <- predictors[which(!is.na(predictors[,1])),, drop = F]
#     #   
#     #   y.fold <- y[which(!is.na(predictors[,1])), , drop = F]
#     #   
#     #   dim(predictors); dim(y.fold)
#     #   colnames(predictors) <- object$methods
#     #   w.yhat <- ensemble(predictors = predictors, y = y.fold, k = n.k)
#     #   
#     #   yhat[which(!is.na(prediction.matrix[, jj[1]])),] <- w.yhat$weighted.yhat
#     # }
#   }
#   ret <- list(yhat = yhat,
#               y = y,
...
# }
# 
# old.subsample <- function(weights, B = 100, strata = NULL, 
#                          train.ratio = 0.8, 
#                          sample.train.ratio = 0.8) {
#   
#   if (is.null(strata)) strata <- gl(1, n)
#   if (!is.factor(strata))
#     stop(sQuote("strata"), " must be a factor")
...
#   
#   # Subfunctions
#   make_stratified_subsample_prediction <- function(n, strata, 
#                                                    train.ratio,
#                                                    sample.train.ratio) {
#     vec <- rep(0, n)
#     
#     if(length(levels(strata))>1){
#       train <- sample(levels(strata), ceiling(train.ratio*length(levels(strata))), replace = F)
#       test.strata <- which(!levels(strata) %in% train)
#       train.strata <- which(levels(strata) %in% train)
#       
#       vec[which(strata %in% train.strata)] <- 0
#       vec[sample(which(vec==0), sample.train.ratio*sum(vec == 0))] <- 1
#       vec[which(strata %in% test.strata)] <- 2
#     }
#     
#     if(length(levels(strata))==1){
#       indx <- c(1:n)
#       vec[sample(indx, sample.train.ratio*train.ratio*n)] <- 1
#       vec[sample(which(vec == 0), (1-train.ratio)*n)] <- 2
#     }
#     vec
#   }
#   
#   # Settings
#   n <- length(weights)
#   folds <- replicate(n = B, make_stratified_subsample_prediction(n, strata = strata,train.ratio, sample.train.ratio))
#   folds <- apply(folds, 2, function(x) x * weights)
#   
#   folds
# }
